...
//    }
	...
	* StringUtils.isBlank("	") = true 为tab键
    ...
    //判断是否包含数字、逗号及字母L
    public static boolean isNumberd(final String input) {
    	Pattern num= Pattern.compile("[0-9-L\\,]*");
    ...
    public static boolean isEmail(final String input) {
    	Pattern num= Pattern.compile("^\\w+((-\\w+)|(\\.\\w+))*\\@[A-Za-z0-9]+((\\.|-)[A-Za-z0-9]+)*\\.[A-Za-z0-9]+$");
    	if(num.matcher(input).matches()){
    		 return true;
    	}
        return false;
    ...
    //判断字符串中数字小于num大于0
    public static boolean isNum(int num,String input){
...
//    /**
//     * 判断传入orderBy字段是否符合
//     * @param orderFields 数组格式:["id","desc"]
//     * @return
//     */
//    public static boolean validOrderByField(String[] orderFields){
//	    if(orderFields.length != 2){
//	        return false;
//        }
//	    String orderBy = orderFields[1];
//        if(ToolUtil.isEmpty(orderBy)){
//            return false;
//        }
